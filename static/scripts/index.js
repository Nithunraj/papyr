...
const Transaction = {
  ...
  total() {
    ...
    total = document.querySelector("#expenseDisplay").innerHTML;
...
const AddChart = {
  ...
  async getData() {
    const expenseData = await Transaction_category.fetchExpenses();

    const labels = Object.keys(expenseData);
    const values = Object.values(expenseData).map(v => v / 100);
